...
/// Connectivity + reachability checks for the pit flow.
class ReachProbe {
  ...
  /// Time-boxed DNS lookup against well-known hosts (never our own domain) so a
  /// VPN or a not-yet-propagated app domain can't produce a false "offline".
  Future<bool> canReachNetwork() async {
    if (!await hasInterface()) return false;
    for (final host in const <String>['apple.com', 'google.com']) {
      try {
        final records =
