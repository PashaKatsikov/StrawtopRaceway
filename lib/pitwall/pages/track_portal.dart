...
/// Full-screen WebView shell for the gray flow. Handles cold-start viewport
/// settling, rotation reflow, offline routing, native-feel JS injections and
/// external-scheme hand-off.
...
class _TrackPortalState extends State<TrackPortal> with WidgetsBindingObserver {
  ...
  Future<void> _settleColdViewport() async {
    ...
    // WebView mounts (no forced rotation). Any residual stretch is corrected by
    // the post-load resize + single reload, in the current orientation.
...
(() => {
  const root = window;
  if (root.__stwInsetKeeper) return;
  root.__stwInsetKeeper = true;
  const marker = 'stw-inset-sheet';
  const rules = [
    ':root{',
...
''');
  ...
  void _installTapPolish() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__stwTapPolish) return;
  window.__stwTapPolish = true;
  const style = document.createElement('style');
  style.id = 'stw-tap-polish';
  style.textContent =
...
''');
  ...
  void _installKeyboardLift() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__stwInputLift) return;
  window.__stwInputLift = true;
...
''');
  ...
  void _installFocusScaleGuard() {
    ...
    _controller.runJavaScript(r'''
(() => {
  if (window.__stwFocusScale) return;
  window.__stwFocusScale = true;
  const style = document.createElement('style');
  style.textContent =
...
''');
  ...
  void _installInlinePlayback() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__stwInlineMedia) return;
  window.__stwInlineMedia = true;
  const awaken = (video) => {
